...
class CreateMembersTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create($table, function (Blueprint $table) {
            ...
            $table->string("avatar", 255)->default("https://scdn.qiqiangkeji.com/210904210553.png")->comment("头像");
            $table->char("invite_code", 8)->unique()->comment("邀请码");
            $table->longText("path")->nullable()->comment("路径[id,pid,ppid]");
            $table->date("birthday")->nullable()->comment("生日");
            $table->unsignedTinyInteger("identity", false)->default(IdentityEnum::PLAIN["key"])->comment("身份");
            $table->unsignedTinyInteger("source", false)->default(SourceEnum::WECHAT_MINI_APP["key"])->comment("来源");
            $table->unsignedTinyInteger("sex", false)->default(SexEnum::SECRET["key"])->comment("性别");
